	options(scipen=999)
	project_directory <- "/lustre/scratch/jmanthey/21_cardellina/07_window_stats_phylo"
	directory_name <- "13_tree_50kbp"
	cluster <- "quanah"
	max_number_jobs <- 400
	
	header_file <- "CM027537.1.recode.vcf.gz"
	
	# read in reference index
	# filtered to only include genotyped chromosomes
	ref_index <- read.table("setophaga.fai", stringsAsFactors=F)
	...
	header <- paste('\tgunzip -cd ', project_directory, '/', header_file, ' | grep "#" > ', project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.recode.vcf", sep="")
